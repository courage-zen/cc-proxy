...
/// - `"codex_oauth"` + "Please re-login via cc-proxy."（cc-proxy 自管 OAuth 路径）
...
async fn refresh_gemini_token(refresh_token: &str) -> Option<String> {
    let client_id = std::env::var("GEMINI_OAUTH_CLIENT_ID")
        .expect("GEMINI_OAUTH_CLIENT_ID environment variable must be set");
    let client_secret = std::env::var("GEMINI_OAUTH_CLIENT_SECRET")
        .expect("GEMINI_OAUTH_CLIENT_SECRET environment variable must be set");
