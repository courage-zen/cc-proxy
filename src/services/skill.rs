...
impl SkillService {
    ...
    /// 获取 SSOT 目录（根据设置返回 ~/.cc-proxy/skills/ 或 ~/.agents/skills/）
    pub fn get_ssot_dir() -> Result<PathBuf> {
        let location = crate::settings::get_skill_storage_location();
        let dir = match location {
            SkillStorageLocation::CcProxy => get_app_config_dir().join("skills"),
    ...
    /// 获取 Skill 卸载备份目录（~/.cc-proxy/skill-backups/）
    ...
    pub fn scan_unmanaged(db: &Arc<Database>) -> Result<Vec<UnmanagedSkill>> {
        ...
        if let Ok(ssot_dir) = Self::get_ssot_dir() {
            scan_sources.push((ssot_dir, "cc-proxy".to_string()));
        }
    ...
    pub fn import_from_apps(
        ...
        search_sources.push((ssot_dir.clone(), "cc-proxy".to_string()));
