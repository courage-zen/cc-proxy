...
//!
//! Note: In the CLI version, usage query via scripts is not supported.
//! Usage data is output as JSON Lines to stdout instead.
...
pub(crate) async fn execute_and_format_usage_result(
    ...
    _user_id: Option<&str>,
    _template_type: Option<&str>,
) -> Result<UsageResult, AppError> {
    // Usage script execution is not supported in CLI mode
    Err(AppError::localized(
        "usage_script.not_supported",
        "用量查询脚本在 CLI 模式下不可用",
        "Usage script execution is not supported in CLI mode",
    ))
}
