...
/// 应用设置结构
///
/// 存储设备级别设置，保存在本地 `~/.cc-proxy/settings.json`，不随数据库同步。
/// 这确保了云同步场景下多设备可以独立运作。
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct AppSettings {
    ...
    /// Skill 存储位置：cc_proxy（默认）或 unified（~/.agents/skills/）
    #[serde(default)]
