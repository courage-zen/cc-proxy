...
impl Database {
    ...
    fn import_sql_string_inner(
        ...
        let sql_content = sql_raw.trim_start_matches('\u{feff}');
        Self::validate_cc_proxy_sql_export(sql_content)?;
    ...
    fn validate_cc_proxy_sql_export(sql: &str) -> Result<(), AppError> {
    ...
    pub(crate) fn backup_database_file(&self) -> Result<Option<PathBuf>, AppError> {
        let db_path = get_app_config_dir().join("cc-proxy.db");
        if !db_path.exists() {
            return Ok(None);
        }
    ...
    fn cleanup_db_backups(dir: &Path) -> Result<(), AppError> {
        ...
        for (entry, _) in sorted.into_iter().take(remove_count) {
