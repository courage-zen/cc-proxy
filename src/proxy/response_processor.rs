...
pub(crate) async fn process_response(
    ...
    if is_sse_response(&response) {
        Ok(handle_streaming(response, ctx, state, parser_config, _connection_guard).await)
    } else {
        handle_non_streaming(response, ctx, state, parser_config, _connection_guard).await
    }
}
...
pub(crate) fn create_logged_passthrough_stream(
    ...
    async_stream::stream! {
        let _conn_guard = _connection_guard;
