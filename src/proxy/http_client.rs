...
fn proxy_points_to_loopback(value: &str) -> bool {
    ...
    // 只有指向自己的代理才需要跳过，避免递归
    fn is_cc_proxy_proxy_port(port: Option<u16>) -> bool {
        let cc_proxy_port = get_proxy_port();
        port == Some(cc_proxy_port)
    }

    if let Ok(parsed) = url::Url::parse(value) {
        if let Some(host) = parsed.host_str() {
            // 只有当主机是 loopback 且端口是 CC Switch 的端口时才返回 true
            return host_is_loopback(host) && is_cc_proxy_proxy_port(parsed.port());
        }
        return false;
    }
