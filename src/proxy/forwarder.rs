...
impl RequestForwarder {
    ...
    async fn forward(
        ...
        // GitHub Copilot 动态 endpoint 路由在 CLI 模式下不支持
        // is_copilot && !is_full_url 时跳过，保留 base_url 不变
        ...
        let mut auth_headers = if let Some(auth) = adapter.extract_auth(provider) {
            // GitHub Copilot 特殊处理：CLI 模式下不可用
            if auth.strategy == AuthStrategy::GitHubCopilot {
                log::error!("[Copilot] GitHub Copilot 认证在 CLI 模式下不可用");
                return Err(ProxyError::AuthError(
                    "GitHub Copilot 认证不可用（CLI 模式不支持）".to_string(),
                ));
            }

            // Codex OAuth 特殊处理：CLI 模式下不可用
            if auth.strategy == AuthStrategy::CodexOAuth {
                log::error!("[CodexOAuth] Codex OAuth 认证在 CLI 模式下不可用");
                return Err(ProxyError::AuthError(
                    "Codex OAuth 认证不可用（CLI 模式不支持）".to_string(),
                ));
            }
