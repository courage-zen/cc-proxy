...
///   且不一定等于用户目录，可能导致 `.cc-proxy/config.json` 路径变化，从而"看起来像数据丢失"。
///
/// ## 测试隔离
///
...
pub fn get_app_config_dir() -> PathBuf {
    ...
    // 使用 XDG 标准目录（Linux/macOS），fallback 到 ~/.cc-proxy
    dirs::config_dir()
        .map(|d| d.join("cc-proxy"))
        .unwrap_or_else(|| get_home_dir().join(".cc-proxy"))
}
